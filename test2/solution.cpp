...
void init(char initialMemberName[], int initialMemberSex) {
  if (vertex_list.size() != 0) vertex_list.clear();

  for (int i = 0; i < 200; i++) {
    if (adjacent_list[i].size() != 0) adjacent_list[i].clear();
  }
...
bool addMember(char newMemberName[], int newMemberSex, int relationship,
  ...
  switch (relationship) {
    ...
    case 1: {
      ...
      // 자신과 성별이 동일한 부모가 존재하지 않음 -> 자신과 기존멤버 사이
      // 부모관계 추가
      if (marriage) {
        // 만약 기존멤버에 자신과 성별이 다른 부모가 존재할 경우 해당멤버와
        // 배우자관계로 추가 자동으로 기존멤버의 부모로 자신이 등록 멤버 추가
        Vertex temp;
        temp.sex = newMemberSex;
        mstrcpy(temp.name, newMemberName);
        vertex_list.push_back(temp);
        ...
        // 만약 기존멤버에 자신과 성별이 다른 부모가 존재하지 않는 경우 바로
        // 자신을 부모로 등록 멤버 추가
        Vertex temp;
        temp.sex = newMemberSex;
        mstrcpy(temp.name, newMemberName);
...
void getDijkstra(char name[]) {
  ...
  while (!pq.empty()) {
    ...
    if (dist_list[currMemNum] < distance) continue;

    for (int i = 0; i < adjacent_list[currMemNum].size(); i++) {
      int destMemNum = adjacent_list[currMemNum][i].first;
      int tempDistance =
          adjacent_list[currMemNum][i].second == 0 ? distance : distance + 1;
...
int countMember(char name[], int dist) {
  ...
  int count = 0;
  for (int i = 0; i < 200; i++) {
    if (i == srcMemNum) continue;

    if (dist_list[i] == dist) count++;
  }
