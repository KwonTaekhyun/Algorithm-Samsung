...
bool cmpStudent(Student std1, Student std2) {
  if (std1.curr < std2.curr) {
    return true;
  } else if (std1.curr == std2.curr) {
    return std1.dest <= std2.dest;
  } else {
    return false;
  }
}
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      Student temp = {curr, dest};
      studentList.push_back(temp);
    }

    studentList.sort(cmpStudent);

    int time = 0;
    while (!studentList.empty()) {
      int limit = studentList.front().dest;
      studentList.pop_front();
      for (list<Student>::iterator itr = studentList.begin();
           itr != studentList.end();) {
        if (limit % 2) {
          if (limit + 1 < (*itr).curr) {
            limit = (*itr).dest;
            itr = studentList.erase(itr);
          } else {
            ++itr;
            continue;
          }
