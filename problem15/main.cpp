...
int zNums[1200002];
bool aList[1200002] = {false};

int calcZeros(bool* aList, int s, int e) {
  if (!aList[s])
    return zNums[e] - zNums[s] + 1;
  else
    return zNums[e] - zNums[s];
}
...
int main(int argc, char** argv) {
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    int maxLength = -1;
    int prevStart = -2;
    for (int start = 0; start <= lastAttend; start++) {
      if ((start == prevStart + 1) && aList[start]) {
        prevStart = start;
        continue;
      }
      ...
      while (l <= r) {
        int m = (l + r) / 2;
        int zeroSum = calcZeros(aList, start, m);
      ...
      maxLength = max(maxLength, lastP - start + 1);

      if (aList[start]) prevStart = start;
    }
