...
int maxCost(int volume, int index) {
  if (index >= N) {
    return 0;
  }

  int& cost = cache[volume][index];

  if (cost != -1) {
    return cost;
  }

  if (volume < itemList[index].volume) {
    cost = maxCost(volume, index + 1);
  } else {
    cost = max(maxCost(volume - itemList[index].volume, index + 1) +
                   itemList[index].cost,
               maxCost(volume, index + 1));
  }
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      Item item = {volume, cost};
      itemList.push_back(item);
    }

    cout << '#' << test_case << ' ' << maxCost(K, 0) << endl;
  }
