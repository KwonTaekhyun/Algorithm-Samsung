...
int main(int argc, char** argv) {
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    switch (String[0]) {
      case 'A': {
        Managers.push_back(0b1000);
        break;
      }
      case 'B': {
        Managers.push_back(0b1100);
        break;
      }
      case 'C': {
        Managers.push_back(0b1010);
        break;
      }
      case 'D': {
        Managers.push_back(0b1001);
        break;
      }
    }
    ...
    for (int i = 1; i < String.size(); i++) {
      switch (String[i]) {
        ...
        case 'B': {
          Managers.push_back(0b0100);
          break;
        }
        case 'C': {
          Managers.push_back(0b0010);
          break;
        }
        case 'D': {
          Managers.push_back(0b0001);
          break;
        }
      }
    }
      ...
      for (int temp = 1; temp < 16; temp++) {
        if ((temp & Manager) != Manager) {
          ...
          for (int curr = 1; curr < 16; curr++) {
            if ((temp & curr)) {
              ret = (ret + Yesterday[curr]) % 1000000007;
            }
          }
          Today[temp] = ret % 1000000007;
        }
      }

      for (int curr = 1; curr < 16; curr++) {
        Yesterday[curr] = Today[curr];
      }
    }
