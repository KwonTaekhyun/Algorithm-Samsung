...
int main(int argc, char** argv) {
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    int StringNumber, ChangeNumber;
    cin >> StringNumber >> ChangeNumber;

    string String = to_string(StringNumber);
    set<string> Set;
    Set.insert(String);
    int MaxPrice = -1;
    ...
    while (ChangeNumber) {
      ...
      for (set<string>::iterator itr = Set.begin(); itr != Set.end(); ++itr) {
        string currString = *itr;

        for (int i = 0; i < currString.size() - 1; i++) {
          for (int j = i + 1; j < currString.size(); j++) {
            string tempString = currString;
            char temp = tempString[i];
            tempString[i] = tempString[j];
            tempString[j] = temp;

            nextSet.insert(tempString);
          }
        }
      }
      Set.swap(nextSet);
    ...
    for (set<string>::iterator itr = Set.begin(); itr != Set.end(); ++itr) {
      MaxPrice = max(MaxPrice, stoi((*itr)));
    }

    cout << '#' << test_case << ' ' << MaxPrice << endl;
  }
