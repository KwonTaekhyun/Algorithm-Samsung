...
long long int countInversion;

void inversion_counting(int s, int e) {
  if (s == e) return;

  int m = (s + e) / 2;
  inversion_counting(s, m);
  inversion_counting(m + 1, e);
...
int main(int argc, char** argv) {
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    inversion_counting(0, N - 1);

    cout << '#' << test_case << ' ' << countInversion << '\n';
  }
