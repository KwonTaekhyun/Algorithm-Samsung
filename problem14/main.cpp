...
vector<String> directory(20000);

bool cmpString(const String A, const String B) {
  ...
  */
  for (test_case = 1; test_case <= T; ++test_case) {
    ...
    for (int i = 0; i < N; i++) {
      string temp;
      cin >> temp;
      directory[i].data = temp;
      directory[i].length = temp.length();
    }

    stable_sort(directory.begin(), directory.begin() + N, cmpString);

    cout << '#' << test_case << endl;

    for (int i = 0; i < N; i++) {
      if ((i != 0) && (directory[i].data == directory[i - 1].data)) {
        continue;
      }

      cout << directory[i].data << '\n';
    }
  }
